class Money:
    def __init__(self, rub, kop):
        # 1. Переводим всю сумму в копейки (единое целое число)
        # Это позволяет корректно работать с отрицательными значениями.
        self._total_kop = rub * 100 + kop

    # --- Свойства для доступа к рублям и копейкам ---
    # Они вычисляются "на лету" из общей суммы.
    @property
    def rub(self):
        # Целочисленное деление сохраняет знак для рублей
        return self._total_kop // 100

    @property
    def kop(self):
        # Остаток от деления всегда возвращает положительное число от 0 до 99
        return abs(self._total_kop % 100)

    # --- Арифметические операции ---
    def __add__(self, other):
        # Складываем общие суммы в копейках
        return Money(0, self._total_kop + other._total_kop)

    def __sub__(self, other):
        # Вычитаем общие суммы в копейках
        return Money(0, self._total_kop - other._total_kop)

    def __mul__(self, other):
        if not isinstance(other, int):
            return NotImplemented
        # Умножаем общую сумму на число
        return Money(0, self._total_kop * other)
    ...
    def __eq__(self, other):
        return self._total_kop == other._total_kop

    def __lt__(self, other):
        return self._total_kop < other._total_kop

    # Остальные сравнения можно добавить для полноты, но для тестов достаточно этих двух.
    # Python сможет вывести их автоматически, но лучше определить явно:
    ...
    def __str__(self):
        # Знак ставим только перед рублями, если вся сумма отрицательная.
        # Копейки всегда выводим как положительное число.

        # Определяем знак для рублей
        sign = '-' if self._total_kop < 0 else ''

        # Используем абсолютное значение рублей для вывода
        abs_rub = abs(self.rub)

        # Формат копеек: если 0, выводим "0коп", иначе "5коп" или "50коп"
        kop_str = f"{self.kop:02d}" if self.kop != 0 else str(self.kop)

        return f"{sign}{abs_rub}руб {kop_str}коп"
